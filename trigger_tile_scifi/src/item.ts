import { TriggerableTileSciFi, TriggerableTileSciFiSystem } from './area'
...
export default class Button implements IScript<Props> {
  init() {
    engine.addSystem(new TriggerableTileSciFiSystem())
  }

  spawn(host: Entity, props: Props, channel: IChannel) {
    const trigger = new TriggerableTileSciFi()
    ...
    const clip = new AnimationState('SciFiTrigger_Action', {
      looping: true,
      speed: 0.5,
    })
    const vanishClip = new AnimationState('SciFiTrigger_Action02', {
      looping: false,
    })
    ...
    tile.addComponent(new GLTFShape('models/sci_fi_trigger.glb'))
