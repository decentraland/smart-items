...
@Component('org.decentraland.WearablesChecker')
export class WearablesChecker {
  lastState: 0 | 1 = 0
...
export class WearablesCheckerSystem implements ISystem {
  group = engine.getComponentGroup(WearablesChecker)

  update(_: number) {
    for (const entity of this.group.entities) {
      const triggerableArea = entity.getComponent(WearablesChecker)
