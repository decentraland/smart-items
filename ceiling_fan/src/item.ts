...
export default class Button implements IScript<Props> {
  clip = new AudioClip('sounds/Fan_Loop.mp3')
  ...
  toggle(entity: Entity, value: boolean) {
    ...
    const activateClip = animator.getClip('Play')
    const deactivateClip = animator.getClip('Stop')
    activateClip.stop()
    deactivateClip.stop()
  ...
  spawn(host: Entity, props: Props, channel: IChannel) {
    ...
    const deactivateClip = new AnimationState('Stop', { looping: true })
    const activateClip = new AnimationState('Play', { looping: true })
